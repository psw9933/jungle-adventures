...
export default class NewClass extends cc.Component {
    ...
    clickLoginBtn(){
        ManagerNotice.getInstance().show('server is not turned on!')
        ...
        httpServer.getInstance().post(url, {
            ...
            if (dataInfo.status == 200) {
                ...
                ManagerNotice.getInstance().show('login success')
            } else {
                cc.log(dataInfo.msg)
                ManagerNotice.getInstance().show('wrong account or password！！')
            }
        })
    }

    clickQuickBtn(){
        cc.systemEvent.emit(gameProtocol.event.showRolePanel, this);
    }
    clickSignUpBtn(){
